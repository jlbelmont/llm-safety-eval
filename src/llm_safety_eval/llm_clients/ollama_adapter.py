...
class OllamaClient(LLMClient):
    """Ollama local inference with opt-in execution guard."""
    ...
    def generate(self, prompt: str, parameters: Dict[str, Any], execute: bool = False) -> GenerationResult:
        ...
        payload = {"model": self.model_name, "prompt": prompt, **parameters}
        with httpx.Client(timeout=30.0) as client:
            resp = client.post(f"{self.provider_config.base_url}/api/generate", json=payload)
